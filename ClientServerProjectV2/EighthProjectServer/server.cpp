...
Server::Server(){
    ConsoleThread* exampleThread = new ConsoleThread;
    exampleThread->start();
    connect(exampleThread, &ConsoleThread::signalStopServer, this, &Server::slotStopServer);
    connect(exampleThread, &ConsoleThread::signalStartServer, this, &Server::startServer);
}
...
void Server::checkLoginAndPassword(const QString &login, const QString &password)
{
    ...
    if (query->exec()) {
        ...
        m_socket->write("{\"type\":\"auth\",\"status\":\"found\",\"name\":\"" + query->value(0).toString().toLocal8Bit() + "\",\"sirname\":\"" + query->value(1).toString().toLocal8Bit() + "\"}");
        } else {
